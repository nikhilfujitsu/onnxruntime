...
void ApplySwiGLUActivation(float* data, int64_t inter_size, bool is_interleaved_format) {
  ...
  if (is_interleaved_format) {
    ...
    for (int64_t i = 0; i < inter_size; ++i) {
      const size_t gate_idx = static_cast<size_t>(2 * i);
      const size_t linear_idx = gate_idx + 1;

      // Store original values
      float gate_val = data[gate_idx];    // Gate: even index (0, 2, 4...)
      float linear_val = data[linear_idx];  // Linear: odd index (1, 3, 5...)

      // Apply clamping to the values
      gate_val = std::min(gate_val, clamp_limit);                        // Clamp gate max only
      linear_val = std::max(std::min(linear_val, clamp_limit), -clamp_limit);  // Clamp linear min/max
      ...
      // Store result in linear element position
      data[linear_idx] = result;
    }
  } else {
    // Non-interleaved format is not implemented yet
    ORT_NOT_IMPLEMENTED("Non-interleaved format for SwiGLU activation is not implemented yet");
  }
}
